    ...
    public function YlReport(){
        ...
        $initSql = "SELECT
                ...
                from (
                    ...
                    sum(x.cntFasheng*p.danjia) as rukuMoney,
                    null as chukuMoney,
                    null as backMoney,
                    null as curCntBack,
                    x.proId as proId
                    from ykb_kucun x
                    left join ykb_base_product p on x.proId=p.id
                    where x.kind in('入库','原料入库','调库入库')  and x.dateFasheng<'{$time[0]}' group by x.proId
                    ...
                    sum(-1*x.cntFasheng*p.danjia) as chukuMoney,
                    null as backMoney,
                    null as curCntBack,
                    x.proId as proId
                    from ykb_kucun x
                    ...
                    sum(x.cntFasheng*p.danjia) as backMoney,
        ...
        $bqSql = "SELECT
            ...
            from (
                ...
                from ykb_kucun x
                left join ykb_base_product p on x.proId=p.id
                where x.kind in ('入库','原料入库','调库入库') and x.dateFasheng>='{$time[0]}' and x.dateFasheng<='{$time[1]}' group by x.proId
                ...
                sum(x.cntFasheng*p.danjia) as chukuMoney,
                null as backMoney,
                null as curCntBack,
                x.proId as proId
                from ykb_kucun x
        ...
        $heji = $this->getHeji($rows, array('initMoney','rukuMoney','chukuMoney','kcMoney'));
        $heji['code'] = '合计';
        $rows[]=$heji;
