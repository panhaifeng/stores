...
use Think\Model;
class YkucunModel extends Model 
{
	//定义主表名称
		...
		if($judge){
			...
			foreach($son as $k => $v){
				$product[] = array(
					...
					'sid'   => $son[$k]['sId']+0,
					'kind'  => $main['rukuKind']?$main['rukuKind']:'入库',
					'proId'	=> $son[$k]['productId'],
					'cntFasheng'	=> $son[$k]['cnt'],
					'priceFasheng'	=> $son[$k]['price'],
					'moneyFasheng'	=> $son[$k]['money'],
					$sonId	=> $son[$k]['id'],
				);
			}
		}else{
			...
			foreach($son as $k => $v){
				$product[] = array(
					...
					'sid'   => $son[$k]['sId']+0,
					'kind'  => $main['chukuKind']?$main['chukuKind']:'出库',
					'proId'	=> $son[$k]['productId'],
					'cntFasheng'	=> $son[$k]['cnt']*-1,
					'priceFasheng'	=> $son[$k]['price'],
					'moneyFasheng'	=> $son[$k]['money']*-1,
					$sonId	=> $son[$k]['id'],
				);
			}
		}
		$row = $product;
		$res = M('kucun')->addAll($row);
		return $res;
	}
		...
		if($judge){
			...
			foreach($son as $k => $val){
				$product[] = array(
					...
					'sid'   => $val['sId']+0,
					'kind'  => $data['rukuKind']?$data['rukuKind']:'入库',
					'proId'	=> $val['productId'],
					'cntFasheng'	=> $val['cnt'],
			...
			foreach($son as $k => $val){
				$product[] = array(
					...
					'sid'   => $val['sId']+0,
					'kind'  => $data['chukuKind']?$data['chukuKind']:'出库',
					'proId'	=> $val['productId'],
					'cntFasheng'	=> $val['cnt']*-1,
					'priceFasheng'	=> $val['price'],
					'moneyFasheng'	=> $val['money']*-1,
					$sonid	=> $val['id'],
				);
			}
		}
	...
	Public function _after_del($id,$fieldName) {
		$map[$fieldName] = $id;
		$res = M('kucun')->where($map)->select();
		foreach($res as &$v){
			$map2['id'] = $v['id'];
			$row = M('kucun')->where($map2)->delete();
		}
		return $res;
	}
}
